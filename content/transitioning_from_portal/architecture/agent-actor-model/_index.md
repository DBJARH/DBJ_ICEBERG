...
Actor placement, fault domains, node topology, and broker placement strategy are covered in [Actor Model Architecture — Physical](/kb/architecture/agent-actor-model/unused/actor-model.html#physical-architecture).

## Implementation

Protobuf schemas, the Train Protocol wire format, and schema governance tooling are covered in [Email Use Case — Implementation](email-use-case.html#implementation).
-->
...
- [Email Management System — Use Case](email-use-case.html) — a worked example applying the `AgentActor` model to organisational email management
