...
{{< callout type="tip" >}}
This document explores how AI Agents may be added externally to a system built on the [Actor Model Architecture](actor-model.html). It is a living document — added to as patterns emerge.
{{< /callout >}}
...
The following are candidate points in the [email use case](/kb/architecture/agent-actor-model/email-use-case.html) where an Agent could add value. None are mandatory — each is a conscious choice to introduce probabilistic behaviour at a specific, bounded location.
...
*This document will grow as patterns are validated. See also: [Actor Model Architecture](actor-model.html) and [Email Use Case](/kb/architecture/agent-actor-model/email-use-case.html) — how AI Agents may be added externally to this system.*
