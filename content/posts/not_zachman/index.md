...
> Cover image is the simplified four-layer version quoted in the claim. It's not what Zachman actually defined — his framework is a 5×6 matrix (Scope/Business/System/Technology/Component crossed with What/How/Where/Who/When/Why), not a single top-to-bottom stack. The linear "Strategic → Logical → Physical → Implementations" progression is a loose paraphrase, which is also why the mapping claim doesn't hold up.


## What's actually there, described by the DBJ Taxonomy

The source is [method.dbj.org/taxonomy_core.html](https://method.dbj.org/taxonomy_core.html). It organizes around a small hierarchy with four top-level categories — things like *Strategic*, *Implementation* — with a "Top category / Capability" pattern used throughout the repo's docs.
...
Any taxonomy that goes from strategy down to implementation will *look* vaguely Zachman-shaped, because that progression is common sense, not a proprietary structure someone has to borrow. Resemblance in shape isn't evidence of derivation.

![alt text](some_shape_different_origin.png)

## The closer near-miss: TOGAF
...
> And that is also true for the whole DBJ Method

That connection isn't accidental, and it isn't "DBJ Taxonomy maps to TOGAF" either. The DBJ Methodology principle is to [**encapsulate** TOGAF](#encapsulation-isnot-referencing) — keep its full apparatus (the [ADM](#adm) phases, artifacts, meta-model) underneath — and expose a simpler, logical "interface" on the surface: DBJ Methodology and DBJ Taxonomy. TOGAF does the heavy lifting out of view; the DBJ taxonomy is the small vocabulary you actually work with day to day.

![DBJ Taxonomy vs ADM](image-1.png)

This is the vocabulary that actually exists: DBJ's own ADM derivative, with three DBJ Taxonomy categories — **Conceptual**, **Logical**, **Physical** — laid directly over the Business/Application/Technology layers, and a fourth, **Implementation**, wrapping the Strategy and Migration phases. That's a real, citable, diagrammed relationship — the taxonomy names sit on top of specific (DBJ) ADM phases, on purpose, as the interface to them.
...
It's tempting — for credibility, or just to sound rigorous — to describe your own taxonomy as "mapping to" a well-known framework. Don't, unless you can point at the sentence where that mapping is asserted.
